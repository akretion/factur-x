...
def pdfgen(args):
    ...
    if isdir(output_pdf_filename):
        logger.error(
            ...
            "Factur-X or Order-X PDF filename)",
            output_pdf_filename,
        )
        sys.exit(1)
    ...
    pdf_metadata = None
    if args.meta_author or args.meta_keywords or args.meta_title or args.meta_subject:
        pdf_metadata = {
    ...
    if isfile(output_pdf_filename):
        if args.overwrite:
            logger.warning(
                "File %s already exists. Overwriting it.", output_pdf_filename
            )
        else:
            logger.error("File %s already exists. Exit.", output_pdf_filename)
            sys.exit(1)
    attachments = {}
    ...
    try:
        ...
        generate_from_file(
            ...
            flavor=args.flavor,
            level=args.level,
            orderx_type=args.orderx_type,
            pdf_metadata=pdf_metadata,
            lang=lang,
            output_pdf_file=output_pdf_filename,
            attachments=attachments,
            afrelationship=args.afrelationship,
...
def main(args=None):
    ...
    parser.add_argument(
        "-nz",
        "--disable-xmp-compression",
        dest="disable_xmp_compression",
        action="store_true",
        help="Disable flate compression of the XMP metadata "
