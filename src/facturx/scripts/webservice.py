...
def generate_facturx():
    ...
    with NamedTemporaryFile(prefix="fx-api-pdf-", suffix=".pdf") as pdf_file:
        with NamedTemporaryFile(
            prefix="fx-api-outpdf-", suffix=".pdf"
        ) as output_pdf_file:
            ...
            generate_from_file(
                pdf_file,
                xml_byte,
                output_pdf_file=output_pdf_file.name,
...
def main(args=None):
    ...
    epilog = "Script written by Alexis de Lattre. " "Published under the BSD licence."
    description = (
        "This is a Flask application that exposes a REST "
    ...
    parser.add_argument(
        "-l", "--logfile", dest="logfile", help="Logs to a file instead of stdout."
    )
    parser.add_argument(
        "-n",
        "--loglevel",
